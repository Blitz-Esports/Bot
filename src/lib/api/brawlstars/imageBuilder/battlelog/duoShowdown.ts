...
export const battleLog_duoShowdown = async (battleLog: ABattlelog) => {
    ...
    //* Draw player info
    if (data.battle.teams.length < 5) return null;
    const playerIcons = await Promise.all(
        [...data.battle.teams[0], ...data.battle.teams[1], ...data.battle.teams[2], ...data.battle.teams[3], ...data.battle.teams[4]]
            .map(async (player) => {
    ...
    const teams = data.battle.teams.reverse().map((team, index) => {
        return {
            team: team.reverse(),
            position: data.battle.teams.length - index
        }
    })

    teams.forEach((teamData, iTeam) => {
        const { team, position } = teamData;
        team.forEach((player, iPlayer) => {
            const starPlayer = iTeam === data.battle.teams.length - 1;
            ...
            ctx.font = "25px Liliita";
            ctx.fillStyle = "white";
            ctx.fillText(`Team: #${position}`, (453.3 * iPlayer) + (20 * (iPlayer + 1)) + ((10 - (iPlayer * 3)) * (iPlayer + 1)) + 273, canvas.height - (133.3 * (iTeam + 1)) - (20 * (iTeam + 1) - (iTeam === 0 ? 6 : 2 * (iTeam + 1))) + 75);
